...
public class VeraPlayer extends VeraEntity implements Player {
    ...
    private void updateBossBars(boolean force) {
        for (BossBar bar : this.bossBars) {
            if (force) {
                this.net().sendPacket(new PlayOutBossBar.Add(bar));
                continue;
            }

            this.net().sendPacket(new PlayOutBossBar.UpdateHealth(bar));
            this.net().sendPacket(new PlayOutBossBar.UpdateTitle(bar));
            this.net().sendPacket(new PlayOutBossBar.UpdateStyle(bar));
            this.net().sendPacket(new PlayOutBossBar.UpdateFlags(bar));
        }
    }
