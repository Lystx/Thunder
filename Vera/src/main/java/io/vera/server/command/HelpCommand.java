...
public class HelpCommand implements CommandListener {
    ...
    private void help(int page, CommandSource source) {
        ...
        int ceil = (int) Math.ceil(VeraServer.getInstance().getCommandHandler().getCmdCount() / (double) PAGE_SIZE);
    ...
    private void aliases(int page, CommandSource source) {
        ...
        int ceil = (int) Math.ceil((VeraServer.getInstance().getCommandHandler().getDispatchers().size() -
                VeraServer.getInstance().getCommandHandler().getCmdCount()) / (double) PAGE_SIZE);
        ...
        int it = 0;
        List<String> help = new ArrayList<>();
        for (Map.Entry<String, CommandDispatcher> entry : VeraServer.getInstance().getCommandHandler().getDispatchers().entrySet()) {
