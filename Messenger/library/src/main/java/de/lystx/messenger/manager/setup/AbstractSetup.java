package de.lystx.messenger.manager.setup;
...
public abstract class AbstractSetup<T> {
     ...
     * @param consumer
     */
    public void start(Console scanner, Consumer<T> consumer) {
        this.consumer = consumer;
        this.console = scanner;
        this.console.setSetup(this);
        scanner.sendMessage("&9");
        scanner.sendMessage("&9");
        if (this.cancellable) {
            scanner.sendMessage("SETUP", "&aIf you want to setup just type &2'cancel'&a!");
        }
        ...
        this.console.sendMessage("SETUP", this.currentPart.getValue().question() + " &7(&a" + this.currentPart.getKey().getType().getSimpleName() + "&7)");
    }

    public void read(String[] args) {
        if (this.current < this.setupParts.size() + 1) {
    ...
    public void next(String lastAnswer) {
        if (this.currentPart != null) {
            if (lastAnswer.trim().isEmpty()) {
                this.console.sendMessage("ERROR", "&cPlease enter a &evalid &cvalue!");
                return;
            }
            ...
            if (!isAnswerAllowed(this.currentPart.getValue(), lastAnswer)) {
                this.console.sendMessage("ERROR", "&cPossible answers: &e"  + Arrays.toString(this.currentPart.getValue().onlyAnswers()).replace("]", "").replace("[", ""));
                return;
            }
            if (isAnswerForbidden(this.currentPart.getValue(), lastAnswer)) {
                this.console.sendMessage("ERROR", !lastAnswer.trim().isEmpty() ? ("&cThe answer '&e" + lastAnswer + " &cmay not be used for this question!") : "&cThis &eanswer &cmay not be used for this question!");
                return;
            }
            ...
            try {
                ...
                this.console.sendMessage("ERROR", "&cPlease enter a valid format");
                return;
            }
        ...
        if (this.currentPart != null)  {
            this.console.sendMessage("SETUP", "&b" + this.currentPart.getValue().question() + " &7(&a" + this.currentPart.getKey().getType().getSimpleName() + "&7)");
        }
    }
