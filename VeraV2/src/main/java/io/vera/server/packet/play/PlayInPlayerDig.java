...
public class PlayInPlayerDig extends PacketIn {
  ...
  public void read(ByteBuf buf, NetClient client) {
    ...
    if (status == DigStatus.START_DIGGING && player.getGameMode() == GameMode.CREATIVE) {
      ...
      if (position.distanceSquared(player.getPosition()) > 36.0D) {
        VeraServer.getInstance().getLogger().warn("Suspicious dig @ " + position + " by " + player
            .getName() + " (" + player.getUuid() + ')');
        return;
      } 
      VeraServer.getInstance().getEventController().callEvent((Event)new PlayerDigEvent((Player)player, block), e -> {
            if (!e.isCancelled())
  ...
  private enum DigStatus {
    START_DIGGING, CANCEL_DIGGING, FINISH_DIGGING, DROP_ITEM_STACK, DROP_ITEM, SHOOT_ARROW_FINISH_EATING, SWAP_ITEM_IN_HAND;
  }
