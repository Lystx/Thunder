    ...
    if (mode.equals("chunks")) {
      ...
      for (int x = playerPosition.getChunkX() - chunkLoadRadius; x <= playerPosition.getChunkX() + chunkLoadRadius; x++) {
        for (int z = playerPosition.getChunkZ() - chunkLoadRadius; z <= playerPosition.getChunkZ() + chunkLoadRadius; z++) {
          Chunk chunk = playerPosition.getWorld().getChunkAt(x, z);
          client.sendPacket((PacketOut)new PlayOutChunk(chunk));
        } 
      } 
      ...
      for (String word : "I hate my life".split(" ")) {
        ...
        bb.setTitle(ChatComponent.text(word).setColor(ChatColor.of((char)(97 + i))));
        bb.setColor(BossBarColor.values()[i]);
        bb.setDivision(BossBarDivision.values()[i++]);
        bb.setHealth(i * 0.25F);
      ...
      RecipientSelector.whoCanSee((VeraEntity)player, false, new PacketOut[] { (PacketOut)new PlayOutDestroyEntities(Collections.singletonList(player)), (PacketOut)addPlayer });
      RecipientSelector.whoCanSee((VeraEntity)player, true, new PacketOut[] { player.getSpawnPacket() });
    } 
  }
}
