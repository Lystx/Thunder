...
public class GameRule<T> {
    ...
    private static GameRule<Boolean> newRule(String name, boolean defValue) {
        GameRule<Boolean> rule = new GameRule<>(name, Boolean.valueOf(defValue), s -> Boolean.valueOf(s.equals("true")));
        GAME_RULES.put(name, rule);
        return rule;
    }

    private static GameRule<Integer> newRule(String name, int defValue) {
        GameRule<Integer> rule = new GameRule<>(name, Integer.valueOf(defValue), Integer::parseInt);
        GAME_RULES.put(name, rule);
        return rule;
    }
    ...
    public static <T> GameRule<T> from(String s) {
        GameRule<T> rule = (GameRule)GAME_RULES.get(s);
        if (rule == null)
            throw new IllegalArgumentException(String.format("NBT value out of range for class %s", new Object[] { "n.t.w.o.GameRule (" + s + ')' }));
        return rule;
    }
